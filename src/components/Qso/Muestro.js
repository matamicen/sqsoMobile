...
import {
  actindicatorPostQsoNewTrue, addMedia,
  ...
  manageLocationPermissions, onprogressFalse, onprogressTrue, postAddMedia,





  postQsoNew, sendActualMedia,
  ...
  setProfileModalStat, setSendingProfilePhotoModal, updateMedia,
...
class Muestro extends Component {
  ...
  rotateImage = async () => {
    ...
    if (this.compressRotation == 86) {
      // const dim = await this.getDimensions(this.props.sqsomedia.url);
      console.log(
        ' entro 1era vez Compress Rotation: ' + this.width + ' ' + this.height
      );

      if (this.width > this.height) valor = this.width;
      else valor = this.height;
      ...
      if (Platform.OS === 'android') coefRotate = coef;
      ...
      console.log(
        ' entro 2da vez Compress Rotation: ' + this.width + ' ' + this.height
      );
    }

    auxoriginalphoto = this.props.sqsomedia.url;
    await ImageResizer.createResizedImage(
      this.props.sqsomedia.url,
      nuevoWidth,
      nuevoHeight,
      'JPEG',
      86,
      ...
      .then((response) => {
        ...
        console.log(
          ' Compress Rotation Rotate resize ImageResizer: ' +
            JSON.stringify(response)
        );

        envio = {
          name: 'fileName2',
          url: this.compressImageURL,
          type: this.props.sqsomedia.type,
          sent: 'false',
          size: this.size,
          width: this.width,
          height: this.height,
      ...
      .catch((err) => {
        ...
        else crashlytics().recordError(new Error('createResizeImg1_PRD'));
      });
    ...
    if (this.props.sqsomedia.type === 'profile') {
      ...
      await ImageResizer.createResizedImage(
        ...
        .then((response) => {
          ...
          console.log(
            ' Compress Rotation Rotate resize AVATAR: ' +
              JSON.stringify(response)
          );
        })
        .catch((err) => {
          ...
          else crashlytics().recordError(new Error('createResizImg2_PRD'));
        });
    }
  };
  ...
  compressImage = async () => {
    ...
    console.log('dim' + dim);
    console.log(
      'tardoMuestro en total obtener info de foto: ' +
        total +
        ' width:' +
        this.width +
        ' height:' +
    ...
    if (this.width > this.height) {
      ...
      if (
        ...
        } else coef = valor / 700; // 725 estaba 650 estaba 570 estaba 450 android, taba 570
      } else coef = valor / 690; // esta coeficiente es para todas las fotos que no vienen de la galeria
    ...
    if (this.props.sqsomedia.gallery && this.props.sqsomedia.type === 'image') {
      ...
      if (Platform.OS === 'ios') compressRate = 80;
      // ios comprimo mas porque vienen con mucha definicion
      else compressRate = 86;
    }
    // nuevoWidthAvatar = this.widthAvatar / 30.0;
    // nuevoHeightAvatar = this.heightAvatar / 30.0;
    if (Platform.OS === 'ios') {
    ...
    await ImageResizer.createResizedImage(
      ...
      .catch((err) => {
        ...
        else crashlytics().recordError(new Error('createResizImg3_PRD'));
      });
    final = new Date();
    total = final - inicial;
    ...
    if (this.props.sqsomedia.type === 'profile') {
      ...
      await ImageResizer.createResizedImage(
        ...
        .catch((err) => {
          ...
          else crashlytics().recordError(new Error('createResiImg4_PRD'));
        });
    }
  };
  ...
  send_and_check_ad = async () => {
    ...
    if (this.props.sqsomedia.type === 'image') {
      if (!this.props.sqsomedia.gallery) {
        ...
        if (this.compressRotation === 86) {
          console.log(
            'entro a comprimir valor de compressRotation: ' +
              this.compressRotation
          );
    ...
    if (this.props.sqlrdsid === '') this.stat = 'waiting';
    else this.stat = 'inprogress';
    ...
    envio = {
      ...
      width: this.width,
      height: this.height,
      qra: this.props.sqsomedia.qra,
      rectime: this.props.sqsomedia.rectime
    };
  ...
  subo_Profile_Photo_s3 = async () => {
    ...
    if (this.compressRotation === 86) {
      console.log(
        'entro a comprimir valor de compressRotation: ' + this.compressRotation
      );
      await this.compressImage();
    } else {
      if (this.rotateCount > 1 && Platform.OS === 'ios') {
  ...
  render() {
    // this.props.imageurl
    console.log(
      'RENDER MUESTRO, mediatosend: ' + JSON.stringify(this.props.sqsomedia)
    );
      ...
      <KeyboardAvoidingView
        ...
        <TouchableWithoutFeedback onPress={Keyboard.dismiss} accessible={false}>
          <View style={{ flex: 1, marginTop: 35 }}>
            <View
              style={{
                flex: this.props.height === 490 ? 0.75 : 0.5,
                ...
                !this.props.sqsomedia.gallery && (
                  <TouchableOpacity onPress={() => this.rotateImage()}>
                    <View style={{ flexDirection: 'row' }}>
                      ...
                      <Text
                        style={{ color: 'orange', fontSize: 11, marginTop: 3 }}>
                        {' '}
                        {I18n.t('MuestroRotate')}
                      </Text>
                    </View>
                  </TouchableOpacity>
                )}
                ...
                <View style={{ flex: 0.3, flexDirection: 'row' }}>
                  ...
                  <View style={{ flex: 0.5, alignItems: 'flex-end' }}>
                    <TouchableOpacity
                      ...
                      onPress={() => this.send_and_check_ad()}>
                      <Text
                        style={{
                          color: 'white',
                          fontWeight: 'bold',
