  ...
  // }  
    ...
    {    
      if (I18n.locale.substring(0, 2) === 'es'){
          ...
          showD = moment(this.state.date).format("D MMM YYYY")
        } 
        else
        {
          showD = moment(this.state.date).format("MMM D YYYY")
      }
     ...
     {(this.state.show && Platform.OS === 'ios' && Platform.Version < '14') && (
        ...
        <View style={{flex: 0.5}} >
         ...
         <DateTimePicker
          ...
          style={{width:'100%', marginLeft: Platform.Version <= '14' ? 110: 0,marginRight: Platform.Version > '14' ? 180:0, marginTop:20}}
          value={this.state.date}
          mode={this.state.mode}
          is24Hour={true}
          display="default"
        ...
        <View style={{flex: 0.5}} >
          ...
          <View style={{flex: 0.5}} >
          <TouchableOpacity  onPress={() => this.selectDatePicker()} >                                       
               <Text style={{ fontSize: 19, color: '#243665',  textAlign: 'right' }} onPress={() => this.selectDatePicker()} >{I18n.t("QsoDateSelect")}</Text>
          </TouchableOpacity>
          </View>
        </View>
