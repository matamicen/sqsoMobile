...
class QsoScreen extends React.PureComponent {
  ...
  static getDerivedStateFromProps(props, state) {
    ...
    console.log(
      'El valor de heightPhotoConfirm : ' + props.sqsomodalconfirmphotoheight
    );
    // this.setState({
  ...
  async componentDidMount() {
    ...
    if (Platform.OS === 'android') this.getAvailablePurchase();
    // }
    ...
    if (nextAppState === 'active') {
      ...
      if (this.props.adshowed || this.props.iapshowed === 1) {
        ...
        if (this.props.photofromgallery === 0)
          ...
          } else console.log('no hay internet');
        else {
  ...
  analyzeVideoPath = async (incomingPath) => {
    ...
    if (Platform.OS === 'android')
      if (realUrl.status) {
        // si bien encontro un path, hay que descartar que sea NO LEIBLE (caso cuando se baja media de WSAPP en la galeria)
        console.log('path:');
        console.log(path);
      ...
      } else pathoculto = true;
    // todo este IF es solo si es Anroid

    if (pathoculto) {
      // unicamente android puede tener un path oculto o que no se pueda acceder(share de whatsapp o galeria de media bajada de whatsapp)
      console.log('path oculto');
      ...
      else this.takeFramePreview(auxuri);
    }
  };
      ...
      if (addMediaCheck('video', this.props.mediafiles)) {
        if (await hasAPIConnection()) {
          ...
          if (fromShare === false) {
            ...
            if (Platform.OS === 'android') {
              // android debe preguntar permiso de storage por el Write del video al disco
              STORAGE_PERMISSION = PERMISSIONS.ANDROID.WRITE_EXTERNAL_STORAGE;
            ...
            console.log(
              'antes de llamar a takeFrame: ' + this.state.videoFromShare
            );
            let storagePermission = true;
  ...
  takeFramePreview = async (videoPath) => {
    ...
    await RNFFmpeg.execute(
      '-y -ss 00:00:01 -i ' + videoPath + ' -vframes 1 -q:v 4 ' + path
    )
      .then((result) => {
        ...
        RNFFprobe.getMediaInformation(videoPath).then((information) => {
          if (information.getMediaProperties() !== undefined) {
            ...
            this.videoDurationSeconds = Math.floor(
              information.getAllProperties().format.duration / 1
            );
    ...
    envio = {
      ...
      rectime: 0,
      gallery: true,
      previewCompressed: this.imagePreviewPath,
  ...
  photoFromShare = async (shareExternalMedia) => {
    ...
    if (storagePermission) {
      ...
      else auxpath = shareExternalMedia;

      RNFetchBlob.fs
        .readFile(auxpath, 'base64')
        // files will an array contains filenames
        .then((files) => {
          console.log('finalizo lectura base64');
          //  console.log(files);
  ...
  photoFromShare_0 = async (shareExternalMedia) => {
    ...
    RNFetchBlob.fs
      .readFile(shareExternalMedia, 'base64')
      // files will an array contains filenames
      .then((files) => {
        console.log('finalizo lectura base64');
        //  console.log(files);
    ...
    ImagePicker.openCropper({
      ...
      .catch((err) => {
        ...
        else crashlytics().recordError(new Error('openCropShareExt_PRD'));
      });
    ...
    if (await hasAPIConnection()) {
      ...
      ImagePicker.openPicker({
        ...
        .catch((err) => {
          ...
          else crashlytics().recordError(new Error('openCropperPOST_PRD'));
        });
    } else this.setState({ nointernet: true });
  };
  ...
  newqso_after_ad = async (qsotype) => {
    ...
    if (1 === 1) {
      ...
      //  }
      if (this.state.externalShareurl) {
        //(this.props.externalshareurl)
        // la app fue llamada desde un share externo
  ...
  receive_data_from_modal = (envio, fileauxProfileAvatar) => {
    ...
    if (
      showIntersitial(this.props.userinfo, envio.type, this.props.mediafiles)
    ) {
    ...
    if (
      showVideoReward(this.props.userinfo, envio.type, this.props.mediafiles)
    ) {
  ...
  subo_s3 = async () => {
    ...
    if ((env.status === 'inprogress') & (this.envio.type !== 'video'))
      // los envia si ya tienen SqlRdsId sino los deja en waiting, cheque !== de video porque puede ser que haya subido primero una foto y luego la haya borrado, entonces quedo el SQLRDSID creado pero el video debe ser comprimido antes de ser enviado
      this.props.uploadMediaToS3(
        this.envio.name,
        this.envio.url,
        fileauxProfileAvatar,
        this.envio.sqlrdsid,
        this.envio.description,
        this.envio.size,
        this.envio.type,
        this.envio.rdsUrlS3,
        this.envio.urlNSFW,
        this.envio.urlAvatar,
        this.envio.date,
        this.envio.width,
        this.envio.height,
        this.props.rdsurls3,
      ...
      if (
        (ONPROGRESS = updateOnProgress(
          ...
          this.props.qsoqras,
          this.auxMedia,
          this.props.activitydatebegin,
          this.props.activitydateend,
          this.props.activityutcbegin,
          this.props.activityutcend
        ))
      )
      ...
      if (this.envio.type === 'video') {
        ...
        this.setState({
          videoCompression: 'inprogress',
          percentageCompressionInitialTime: new Date(),
          videoSizeBeforeCompress: this.envio.size
        });

        console.log(
          'this.videoPathBeforeCompress: ' + this.videoPathBeforeCompress
        );
        // mpeg4
        inicioCom = new Date();
        ...
        if (needTrim)
          setTimeout(() => {
            ...
            if (Platform.OS === 'ios')
              // ios comprime mucho mas entonces hay que darle mas definicion para que no aruine los videos
              compress = ' -vf scale=-2:400 ';
            else compress = ' -vf scale=-2:320 ';
            ...
            RNFFmpeg.executeAsync(
              ...
              (completedExecution) => {
                ...
                if (completedExecution.returnCode === 0) {
                  ...
                  RNFFprobe.getMediaInformation(destination_path).then(
                    (information) => {
                      if (information.getMediaProperties() !== undefined) {
                        let streams = information.getStreams();
                        console.log(
                          `size papa: ${
                            information.getAllProperties().format.size
                          }`
                        );
                        console.log(
                          `duration papa: ${Math.floor(
                            information.getAllProperties().format.duration / 1
                          )}`
                        );
                        console.log(
                          `Width papa: ${streams[0].getAllProperties().width}`
                        );
                        console.log(
                          `Height papa: ${streams[0].getAllProperties().height}`
                        );

                        this.envio.rectime = Math.floor(
                          information.getAllProperties().format.duration / 1
                        ); // duration;
                        ...
                        update = {
                          rectime: this.envio.rectime,
                          size: this.envio.size,
                          url: this.envio.url,
                          width: streams[0].getAllProperties().width,
                          height: streams[0].getAllProperties().height
                        };
                        this.props.updateCommentInMemory(
                          this.envio.name,
                          update
                        );
  ...
  publicar_chequeos = async () => {
    if (this.pressPublish === false) {
      ...
      if (ONPROGRESS) {
        if (await hasAPIConnection()) {
          ...
          if (this.envio.type === 'video') {
            ...
            if (this.props.sqsosqlrdsid === '') {
              ...
              console.log(
                'despues de await postQsoNew, el awaiy de postQsoNew es clave para que no se tave con el delay de 1 segundo de abajo'
              );
            ...
            while (this.state.videoCompression === 'inprogress') {
              ...
              RNFFmpegConfig.getLastReceivedStatistics().then((statistics) => {
                ...
                percentage = Math.floor(
                  ((timeCodeProcesado / this.videoDurationSeconds) * 100) / 1
                );
            ...
            contEnvio = 0;
            while (
              todaMediaEnviadaAS3(this.props.mediafiles) === false &&
              contEnvio < 30
            ) {
              /* code to wait on goes here (sync or async) */
              contEnvio++;
        ...
        console.log(
          'Todavia no esta OnProgreSSS como para llamar a PostNewQso'
        );
        this.pressPublish = false;
        console.log(
          missingFieldsToPublish(
        ...
        missMessage = missingFieldsToPublish(
          this.props.qsotype,
          this.props.band,
          this.props.mode,
          this.props.qsoqras,
  ...
  publicar = async (jwtToken) => {
    ...
    if (
      ...
      console.log(
        'parse: ' +
          realdate_aux.getFullYear() +
          ' ' +
          realdate_aux.getMonth() +
          ' ' +
          realdate_aux.getDay() +
          ' ' +
          realtime_aux.getHours() +
          ' ' +
          realtime_aux.getMinutes()
      );
      ...
      RNFetchBlob.fs
        ...
        .then(() => {
          ...
          if (Platform.OS === 'android' && Platform.Version < 30) {
            // el scanFile funciona solo en android
            RNFetchBlob.fs
              .scanFile([this.file10_delete, this.videoaux_delete])
  ...
  render() {
    ...
    return this.props.sqsonewqsoactive ? (
      <HandleBack onBack={this.onBack}>
        <View style={{ flex: 1, backgroundColor: '#fff' }}>
          <View style={{ flex: 0.06 }}>
            <Publicar
              publicar={this.publicar_chequeos.bind()}
              descartar={this.descartar_publicacion.bind()}
            />
          </View>
            ...
            <Modal
              ...
              <View
                ...
                {this.state.videoCompression === 'inprogress' &&
                  this.props.mediafiles[0].type === 'video' && (
                    <Text
                      style={{
                        color: 'yellow',
                        // fontWeight: "bold",
                      ...
                      {I18n.t('QsoScrCompressingVideo')}{' '}
                      {this.state.videoCompressPercentage}%
                    </Text>
                  )}
                {this.state.videoCompression === 'finished' &&
                  ...
                  this.state.videoPercentage !== -1 && (
                    <Text
                      ...
                      {I18n.t('QsoScrUploadingVideo')}{' '}
                      {this.state.videoPercentage}%
                    </Text>
                  )}
         ...
         <VariosModales
          ...
          />  */}
            ...
            <Modal
              ...
              <View
                ...
                <Iap />
                <Button
                  onPress={() => this.setState({ iap: false })}
                  title="Cierro"
                />
              </View>
            </Modal>
            ...
            <Modal
              ...
              <View
                ...
                <Muestro
                  send_data_to_qsoscreen={this.receive_data_from_modal.bind()}
                  height={this.state.heightPhotoConfirm}
                ...
                <TouchableOpacity
                  ...
                  <Text
                    style={{
                      fontSize: 13,
                      color: 'black',
                      marginLeft: I18n.locale.substring(0, 2) === 'es' ? 16 : 14
                    }}>
                    {I18n.t('QsoScrPhoto')}
                  </Text>
                </TouchableOpacity>
          ...
          {this.state.showIntersitial && (
            <AdInter
              newqso={this.newqso_after_ad.bind()}
              subos3={this.subo_s3.bind()}
              closead={this.closeAd}
            />
          )}
          {this.state.showVideoReward && (
            <AdVideoReward
              newqso={this.newqso_after_ad.bind()}
              subos3={this.subo_s3.bind()}
              closead={this.closeAd}
              notrewared={this.not_rewarded.bind()}
            />
          )}
          ...
          {this.state.deletePost && (
            <DeletePost
              sqlrdsid={this.props.sqsosqlrdsid}
              closeDeletePost={this.CloseDeletePost.bind()}
              deleteqsofolder={this.deleteQSOfolder.bind()}
            />
          )}
          ...
          {this.state.missingFields && (
            <MissingFieldsToPublish
              ...
              closeMissingFields={this.closeMissingFields.bind()}
            />
          )}
        </View>
      </HandleBack>
      ...
      <View style={{ flex: 1 }}>
        <View style={{ flex: 0.1, alignItems: 'center', marginTop: 10 }}>
          <Text style={{ color: '#243665', fontWeight: 'bold', fontSize: 22 }}>
            {I18n.t('QsoTypeSelect')}{' '}
          </Text>
        </View>
        ...
        <View
          ...
          <TouchableOpacity
            ...
            onPress={() => this.newQso('QSO')}>
            <View
              style={{ flexDirection: 'row', flex: 1, alignItems: 'center' }}>
              <Image
                source={require('../../images/qso10.png')}
        ...
        <View
          ...
          <TouchableOpacity
            style={{ height: 85, marginTop: 1 }}
            onPress={() => this.newQso('QAP')}>
            <View
              style={{ flexDirection: 'row', flex: 1, alignItems: 'center' }}>
              <Image
...
const mapStateToProps = (state) => {
  ...
  return {
    ...
    sqsoactindicatorpostqsonew:
      state.sqso.currentQso.activityindicatorPostQsoNew,
